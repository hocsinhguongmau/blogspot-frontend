...
const PostPage = (props: PostType) => {
  ...
  const { isLoading, isError, error, data }: UseQueryResult<PostType | undefined, Error> = useQuery<
    PostType | undefined,
    Error
    ...
    if (data.id) {
      ...
      return (
        <>
          ...
          <div className="post">
            <div className="breadcrumbs" style={{ backgroundImage: `url('${imageProps.src}')` }}>
                ...
                <div className="recent-posts__wrapper">
                  {data.categories.map((category) =>
                    category.relatedPosts.map((post: RelatedPostType) => (
                      <RelatedPost post={post} key={post.id} classes="recent-posts__item" size={[150, 100]} />
                    ))
                  )}
                </div>
              </div>
