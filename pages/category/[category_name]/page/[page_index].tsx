...
const CategoryPage = (props: PostPageType) => {
  ...
  const { isLoading, isError, error, data }: UseQueryResult<PostPageType | undefined, Error> = useQuery<
    ...
    Error
  >([`postsByCategory_${category}`, page], () => getPostsByCategory(category, start, end), {
    initialData: props,
  });
  ...
  if (!data) {
    ...
    useEffect(() => {
      if (numberOfPosts && postsPerPage * page < numberOfPosts) {
        queryClient.prefetchQuery<PostPageType | undefined, Error>([`postsByCategory_${category}`, page + 1], () =>
          getPostsByCategory(category as string, page * postsPerPage, (page + 1) * postsPerPage)
        );
      }
...
export const getServerSideProps: GetServerSideProps = async ({ query }: GetServerSidePropsContext) => {
  ...
  const data = await getPostsByCategory(query?.category_name as string, start, end);
